...
//window 扩展
/*
//安装  xlsx.core.min.js 后使用
利用 excel.js 将数组导成excel 
*/
(function () {
    window.xlsxex = (function () {
        var ex = {
            LoadJS: function (src) {
            ...
            //sheets:{sheet1name:[{}]}
            downXlsxFileByArray: function (sheets, name) {
                var wb = XLSX.utils.book_new();
                for (var i in sheets) {
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(sheets[i]), i);
                }
                var workbookBlob = this.workbook2blob(wb);
                this.openDownloadDialog(workbookBlob, name);
            }

        }
        var spt = document.getElementsByTagName("script");
        ex.LoadJS(spt[spt.length - 1].src.replace("xlsx.ex.js", "") + "xlsx.core.min.js");
        return ex;
    })();
